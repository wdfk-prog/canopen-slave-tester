...
This validation process checks the CANopenNode CiA 304 SRDO protocol implementation on the
...
SocketCAN interface and passes it to the enabled GFC protocol validation and SRDO protocol validation
...
process may consume it. SRDO protocol validation drains the channel before its first capture so
frames left by an earlier safety-protocol process cannot become SRDO evidence. The fixture
