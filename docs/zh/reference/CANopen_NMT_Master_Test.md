...
与 automatic CANopen validation suite 不同，本阶段 Linux 端不是 CANopen Master。Linux 程序编译为 Lely `BasicSlave`，作为被 MCU 控制的真实 CANopen 从节点。
...
CANOPEN_ROLE_MASTER
    -> AsyncMaster
    -> automatic CANopen validation suite
...
Lely Slave callback 则证明该 NMT 命令已经被 CANopen 协议栈接受并进入相应 NMT 状态迁移流程，因此作为 NMT master validation 主断言更合适。
...
- Master process table 只注册当前 enable 宏开启的 enabled validation process，并由 `canopenRunProcesses()` 按固定顺序执行；GFC protocol validation 启用时排在 EMCY consumer validation 后；
...
2. 已启用验证流程保持固定相对顺序；EMCY consumer validation 位于 EMCY producer validation 后，GFC protocol validation 启用时位于 EMCY consumer validation 后；
3. NMT Master 流程不应出现在 Master 自动测试日志中。

## 9. 当前未验证边界

当前代码侧验证不等价于目标板 NMT master validation PASS。最终仍需要：
