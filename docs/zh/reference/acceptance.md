# automatic CANopen validation suite 自动测试验收
...
确认 PDO/SYNC validation 没有绕过 Lely 构造 SocketCAN PDO/SYNC 帧：
...
2. Heartbeat validation 稳定等待覆盖 5 个 500 ms Heartbeat 周期；
...
1. PDO validation 发送前先通过 SDO 保存 `0x2200:00`；
...
8. 如果 SDO local completion wait 状态未知，只允许 best-effort PDO 恢复，PDO validation 仍判定失败，不得宣称清理 PASS。

## PDO validation NMT 与清理

PDO validation 开始时本地主站 Node-ID 127 切到 Operational，并向节点 1发送 NMT Start。结束时必须：

- 注销 PDO validation 的 `OnRpdo()`/`OnTpdo()` callback；
- 本地主站保持 Operational，不得因阶段清理触发广播 Reset Communication；
- 从机 `0x2200:00` 恢复并验证原值，或明确报告恢复未验证；
- 不修改 EDS、DCF 或持久化参数。

## SYNC/synchronous PDO validation 与同步 TPDO1
...
4. Enter Pre-operational 命令后必须观察到新的远端 `OnState(PREOP)` 事件，再按 disable -> transmission type 1 -> enable 顺序配置成功并回读；若该状态确认超时，本次 SYNC/synchronous PDO validation 判 FAIL，且在尚未修改 TPDO1 的情况下仍必须执行最终 Start 清理；
...
13. 本地主站 `0x1006` 恢复为 SYNC/synchronous PDO validation 进入前原值；
...
前置：从机实际固件必须启用 `PKG_CANOPENNODE_EM_PROD_CONFIGURABLE`、`PKG_CANOPENNODE_EM_PROD_INHIBIT` 和 `PKG_CANOPENNODE_EM_HISTORY`；初始 `0x1001` 必须为 0。EMCY producer validation 不修改 `0x1016`。
...
前置：MCU 固件启用 `PKG_CANOPENNODE_EM_CONSUMER` 与 `PKG_CANOPENNODE_DEMO_EMCY_CONSUMER_DIAGNOSTIC`，并提供只读 `0x2301:01..07`。Host 必须使用 Master role Node 127，local `0x1014` 对应标准 EMCY CAN-ID `0x0FF`，`0x1015=0`，且进入 EMCY consumer validation 前没有预存的 Host local EMCY。生成的 compact `0x1003` 在 Lely EMCY service 第一次同步前可能仍反映 DCF 的初始 compact 值，因此 preflight 不把初始 `0x1003:00` 当作 active stack 深度。
...
9. 任一失败路径都必须清除 EMCY consumer validation 自己产生的 Host local EMCY；调用全栈 `clear()` 前必须确认 `0x1003:00` 深度与 EMCY consumer validation 未清 error 数一致、栈顶仍是 EMCY consumer validation error 且 combined Error Register 为 `0x81`，发现其他 Host fault 时必须拒绝清栈；如果已经执行 Reset Communication 且 fresh Boot 尚未证明 MCU Operational，必须尝试恢复 MCU Operational；
10. EMCY consumer validation 不写 MCU `0x1014/0x1015/0x1016`，不增加第二节点，不使用 `cansend`/Raw CAN。

辅助 `candump` 应能观察 `0x0FF` 的非零 error EMCY 与 `0x0000` recovery；EMCY consumer validation 自动 PASS 以 MCU `0x2301` SDO snapshot 和 Boot/NMT evidence 为准。完整目标板 HIL 还需结合现有全局 CAN state 日志和 CAN 抓包确认测试期间未进入 passive/bus-off，该项不由 `emcyConsumerProcess()` 的自动返回码判定。
...
8. `0x1000:00` SDO health 成功；退出 GFC protocol validation 前确认 producer request/complete 无 pending，恢复测试前 `0x1300` 并回读。

`gfcProcess()` 的 GFC protocol-health regression 只做上述 SDO smoke；GFC integration validation 完整验收还必须在 GFC protocol validation 后按当前选定的 core-protocol regression profile 重新执行既有 core-protocol 测试。未执行该回归时不得声明“GFC 不影响普通 core-protocol”已验证。完整目标板 HIL 还需结合 CAN state 日志和独立抓包确认没有 passive/bus-off。本阶段只声明 GFC protocol functional verification，不声明 SIL、PL 或安全认证。
...
| 2 | Heartbeat validation 主站 Heartbeat timeout | 节点 1 上报 `0x8130` |
| 3 | Heartbeat validation 主站 Heartbeat recovery | 节点 1 上报 `0x0000` |
| 4 | Heartbeat validation 从机 timeout/recovery | `OnHeartbeat(..., true/false)` 均在 3000 ms 内发生 |
| 5 | SDO object-access validation write/read-back | `0x2200` probe 一致 |
| 6 | SDO object-access validation restore | 原值恢复并回读一致 |
| 7 | PDO validation TPDO receive | 5 帧、DLC 8、无 processing error |
| 8 | PDO validation TPDO timing | 4 个间隔均为 1000+/-150 ms |
| 9 | PDO validation TPDO mapping | raw、Lely mapped、SDO OD 一致 |
| 10 | PDO validation RPDO transmit | `0x201` DLC 4，payload 等于 probe |
| 11 | PDO validation RPDO apply | SDO `0x2200` 回读等于 probe |
| 12 | PDO validation cleanup | `0x2200` 原值恢复并验证，本地主站保持 Operational，且不触发广播 Reset Communication |
| 13 | SYNC/synchronous PDO validation topology | 主站 producer、从机 consumer，COB-ID/0x1019 一致 |
| 14 | SYNC/synchronous PDO validation quiet window | 无 SYNC 时无同步 TPDO1 |
| 15 | SYNC/synchronous PDO validation sync/TPDO timing | 5 SYNC 对应 5 TPDO1，均位于对应同步周期 |
| 16 | SYNC/synchronous PDO validation cleanup | 0x1800 全快照恢复并回读，事件型 TPDO 周期恢复 |
| 17 | EMCY producer validation state/history | 0x8130/0x0000 与 0x1001/0x1003 一致，无 active-error 重复 EMCY |
| 18 | EMCY producer validation configurable COB-ID | slave 0x1014 与 master 0x1028:01 都切到 0x681 并可收到 EMCY |
| 19 | EMCY producer validation inhibit | 0x1015=15000 时 fault/reset 间隔 1400..2500 ms |
| 20 | EMCY producer validation cleanup | 0x1014/0x1015/0x1028/0x1017 全部恢复并回读，原 COB-ID smoke test 通过 |
| 21 | EMCY consumer validation single/consecutive EMCY | vector A/B 各产生一次精确 `remote_rx_count +1`，全部字段一致 |
| 22 | EMCY consumer validation recovery/duplicate | recovery 为全零 payload；重复 A+A 各自计数，不去重 |
| 23 | EMCY consumer validation SDO health | EMCY callback 后 `0x1000:00` upload 成功 |
| 24 | EMCY consumer validation reset rebind | RESET_COMM + fresh Boot 后，0x2301 count/last snapshot 保持不变，再收到一次 vector B 与 recovery |
| 25 | EMCY consumer validation cleanup | Host local EMCY stack 清理；Reset 后 MCU 恢复 Operational |
| 26 | GFC protocol validation consumer gates | valid=1 精确 +1；valid=0、DLC1 不增加；`0x1300=2` 返回 `PARAM_VAL (0x06090030)` 且不改值 |
| 27 | GFC protocol validation producer wire | valid=0 无 `0x001`；valid=1 捕获标准 `0x001/DLC0` 且 result=0 |
| 28 | GFC protocol validation continuous/reset | 连续 3 帧逐次 +1；RESET_COMM 后 count 保持且 fresh GFC +1 |
| 29 | GFC protocol validation cleanup/health | `0x1000` SDO 成功，producer 无 pending，原 `0x1300` 恢复并回读，MCU Operational |
| 30 | GFC integration validation core-protocol regression | GFC protocol validation 后重新执行当前选定的既有 core-protocol regression profile 并通过 |
...
3. Master process table 只包含当前启用的 enabled validation process，并由 `canopenRunProcesses()` fail-fast 执行；GFC protocol validation 启用时固定追加在 EMCY consumer validation 后；
