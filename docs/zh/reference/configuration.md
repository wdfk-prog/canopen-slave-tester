...
/* 或 */
#define CANOPEN_ROLE CANOPEN_ROLE_MASTER /* automatic CANopen validation suite */
```
...
| 宏 | 默认值 | 说明 |
| --- | ---: | --- |
| `CANOPEN_ROLE` | `CANOPEN_ROLE_MASTER` | 直接在 `include/canopen_config.h` 修改；`SLAVE` 用于 NMT HIL，`MASTER` 用于 enabled validation process |
...
| `CANOPEN_SLAVE_NODE_ID` | `1` | Master role enabled validation process 的被测 MCU Node-ID |
...
| `CANOPEN_WAIT_TIMEOUT_MS` | `5000` | Startup Boot、SYNC/synchronous PDO validation 异常恢复和 Final Reset 等跨流程等待时间 |
...
| `include/nmt_heartbeat.h` | `CANOPEN_ENABLE_HEARTBEAT_PROCESS` | `0` | 当前不注册 Heartbeat validation |
| `include/sdo_block_process.h` | `CANOPEN_ENABLE_SDO_BLOCK_PROCESS` | `1` | 注册并执行 SDO server block-transfer validation |
| `include/sdo_process.h` | `CANOPEN_ENABLE_SDO_PROCESS` | `0` | 当前不注册 SDO object-access validation |
| `include/pdo_process.h` | `CANOPEN_ENABLE_PDO_PROCESS` | `0` | 当前不注册 PDO validation RPDO/TPDO |
| `include/sync_pdo_process.h` | `CANOPEN_ENABLE_SYNC_PDO_PROCESS` | `0` | 当前不注册 SYNC/synchronous PDO validation/同步 TPDO |
| `include/time_process.h` | `CANOPEN_ENABLE_TIME_PROCESS` | `0` | 当前不注册 TIME consumer validation；实现仍保留 |
| `include/emcy_process.h` | `CANOPEN_ENABLE_EMCY_PROCESS` | `1` | 注册并执行 EMCY producer validation 测试 |
| `include/emcy_consumer_process.h` | `CANOPEN_ENABLE_EMCY_CONSUMER_PROCESS` | `1` | 注册并执行 EMCY consumer validation MCU EMCY Consumer 测试 |
| `include/gfc_process.h` | `CANOPEN_ENABLE_GFC_PROCESS` | `1` | 注册并执行 GFC protocol validation consumer/producer 测试 |
| `include/sdo_client_process.h` | `CANOPEN_ENABLE_SDO_CLIENT_PROCESS` | `1` | 注册并执行 MCU SDO client validation |
| `include/sdo_client_process.h` | `CANOPEN_ENABLE_SDO_CLIENT_BLOCK_REGRESSION` | `0` | MCU SDO client block-transfer regression 条件回归；仅在 `CANOPEN_ENABLE_SDO_CLIENT_PROCESS=1` 时有效；MCU 需 `PKG_CANOPENNODE_DEMO_SDO_CLIENT_TEST=y` + `PKG_CANOPENNODE_SDO_CLI_BLOCK=y` |
...
```text
Heartbeat validation  src/nmt_heartbeat.cpp
SDO server block-transfer validation src/sdo_block_process.cpp
SDO object-access validation  src/sdo_process.cpp
PDO validation  src/pdo_process.cpp
SYNC/synchronous PDO validation  src/sync_pdo_process.cpp
TIME consumer validation  src/time_process.cpp
EMCY producer validation  src/emcy_process.cpp
EMCY consumer validation  src/emcy_consumer_process.cpp
GFC protocol validation src/gfc_process.cpp
MCU SDO client validation src/sdo_client_process.cpp
```

Heartbeat validation 当前使用 `kHeartbeatIndex=0x1017`、`kHeartbeatTimeoutMs=3000`、`kHeartbeatSampleCount=5` 和 500 ms Producer Heartbeat 周期。SDO object-access validation 使用 `kTestObjectIndex=0x2200`、`kProbeValue=0x12345678`，并在 probe 与原值相同时选择备用值。PDO validation 的 PDO number、`0x2100/0x2101/0x2200`、采样策略和时序容差全部保存在 `pdo_process.cpp`；SYNC/synchronous PDO validation 的 SYNC 周期、样本数量、TPDO1 参数索引和时序窗口保存在 `sync_pdo_process.cpp`；TIME consumer validation 的 `0x1012/0x2300`、TIME 边界值、诊断轮询和时序容差保存在 `time_process.cpp`；EMCY consumer validation 的 `0x2301`、EMCY vectors、500 ms SDO timeout、2 s 观察窗口和 snapshot 重试次数都保存在 `emcy_consumer_process.cpp`。GFC protocol validation 的 `0x1300/0x2302`、固定 CAN-ID `0x001`、负向观察窗口和 wire timeout 都保存在 `gfc_process.cpp`。

从机的基线 Heartbeat 和 PDO 参数仍由 YAML、EDS、`dcfgen` 和 Lely Boot 管理。修改真实通信参数时必须同步修改配置源并重新生成 DCF；源码中的流程私有常量只定义测试行为。EMCY consumer validation 不修改 Host/MCU EMCY COB-ID 或 inhibit 参数；Host `0x1014` 必须保持标准 Node 127 EMCY CAN-ID `0x0FF` 且 `0x1015=0`，MCU 固件必须提供只读 `0x2301` diagnostic。
