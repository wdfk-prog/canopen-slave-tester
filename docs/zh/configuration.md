...
| `CANOPEN_PEER_HEARTBEAT_MS` | `500` | 软件 peer Heartbeat 周期 |
...
只有明确需要本机 native Linux 构建且存在兼容 Lely 安装时才使用 `CANOPEN_NATIVE_BUILD=ON`。CI 和 Release 不使用该模式，而是使用真实 TQ8MP 交叉工具链。
